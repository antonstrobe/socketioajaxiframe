...
var sendButton = document.getElementById("df0"); sendButton.addEventListener("click", sendMessage); socket.on('chat', function(data){displayNewMessage(data.message);}); 

function sendMessage() {socket.emit('chat', {message: df0.value} );} function displayNewMessage (message) {
...
function sendMessage100(event){socket.emit('chat',{message: df100.value} );}function displayNewMessage100 (message) {window.top.postMessage(message);}
